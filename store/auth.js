...
export const actions = {
    ...
    async updateProfile({ commit, rootState }, { firstName, lastName, email, city, zip, avatar, gender, dob, language, restaurant, dish1, dish2, dish3 }) {
        if (rootState.settings.demo) {
            commit('info', 'Demo mode: Unable to update profile info', { root: true })
            return
        }
        try {
            const data = await this.$axios.$put('/users/profile', { firstName, lastName, email, city, zip, avatar, gender, dob, language, restaurant, dish1, dish2, dish3 })
            if (data) {
